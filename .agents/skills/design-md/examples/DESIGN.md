...
> [!IMPORTANT]
> **Strict Consistency Rule:** Fonts must be perfectly synchronized across all heading levels (H1 to H6) across all pages of the application. For example, if an H1 is defined as `text-3xl` on desktop, it must remain `text-3xl` on all other pages. No custom, arbitrary fonts or ad-hoc heading sizes are allowed to be added to the project.

### Hierarchy & Weights

- **Display Headlines (H1):** Semi-bold weight (600), generous letter-spacing (0.02em for elegance), 2.75-3.5rem size on desktop. Must remain absolutely consistent across all screens.
- **Section Headers (H2):** Semi-bold weight (600), subtle letter-spacing (0.01em), 2-2.5rem size on desktop. Establishes clear content zones and featured collections.
...
- **Font-to-Padding Synchronization:** Padding values across components (buttons, input fields, badges) are strictly synchronized with their respective font sizes to ensure balanced geometric proportions.
...
- **Line-Wrapping Rule:** All primary and secondary buttons, tabs, links, and action elements **MUST strictly use `whitespace-nowrap`**. They must stay perfectly aligned on a single line and are strictly forbidden from breaking or wrapping to multiple lines.
...
- **Aggressive Size & Scale Reduction:** When transitioning to mobile/tablet screens, typography sizes and app paddings **MUST shrink to the absolute minimum acceptable values**. For example, large desktop headers (e.g., `3xl` or `4xl`) must scale down to standard readable sizes (like `lg` or `xl`) rather than remaining oversized (`2xl`) on mobile. Padding and margins must similarly compress to maximize content real estate.
- **Mobile 2-Column Grid Exception:** While highly complex cards with interactive components default to a single column (`grid-cols-1`) on mobile, simpler cards containing less information (such as blogs, projects, simple lists) **MUST render in 2 columns (`grid-cols-2`) on mobile** to maintain visual density.
- **Ultra-Dense Mobile Styling:** Treat 2-column mobile cards with the layout logic of a dedicated native app:
  - Hide all secondary or non-essential details (e.g., descriptions, excerpts, redundant meta tags).
  - Shrink internal card padding (`p-2` or `p-3`) and reduce font sizes (`text-xs` or `text-[10px]`) to the absolute minimum readable limit to prevent awkward word-wraps and horizontal overflow.
- **Systematic Viewport Verification:** All page layouts, components, and text flow must be thoroughly verified across all responsive dimensions (from 320px width up to 4K desktop) to completely prevent any text overflow, horizontal scroll, overlapping elements, or other UI breakage.
...
- **Body Scroll Lock for Overlays:** When any modal, popup, dialog, drawer, or sidebar is active/opened, the main page body background scroll **MUST be disabled** (`document.body.style.overflow = "hidden"`). The scroll lock must be safely restored to the original value when the overlay closes/unmounts to prevent scroll leakage.
