...
export default function TemplatePreview({
    ...
    return (
      <div className="group relative rounded-3xl border border-red-800 bg-red-900/20 p-4">
        <p className="text-sm text-red-400">
          Component không tìm thấy: {componentName}
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="group relative rounded-3xl border border-slate-800 bg-slate-900/40 p-4 shadow-lg transition-all hover:bg-slate-900/60">
      <div className="mb-4 flex items-center justify-between px-2">
        <h3 className="text-sm font-semibold text-slate-400 capitalize">
          {componentName.replace("-", " ")}
        </h3>
        <span className="rounded-full bg-indigo-500/10 px-2 py-1 text-[10px] font-bold text-indigo-400 ring-1 ring-indigo-500/20 ring-inset">
          Interactive
        </span>
      </div>

      <div className="relative flex min-h-[200px] items-center justify-center overflow-hidden rounded-2xl border border-slate-800/50 bg-slate-950">
        <Suspense
          fallback={
