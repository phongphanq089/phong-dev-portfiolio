import TemplatePreview from "@/features/studio/components/template-preview"
    ...
    <div className="space-y-16">
      ...
      <section className="max-w-4xl space-y-6">
        ...
        <p className="max-w-2xl text-xl leading-relaxed text-slate-400">
          A sandbox for high-end UI components and interactive templates. The
          shell is pre-rendered, while the interactive modules below are
          hydrated on-demand.
        </p>
      </section>
      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-2 xl:grid-cols-3">
        {templates.map((template) => (
          <TemplatePreview key={template.uuid} componentName={template.id} />
        ))}
      </div>

      {/* Footer info - SSR safe */}
      <footer className="border-t border-slate-800 pt-12 text-sm text-slate-500">
